...
import { formatDate, daysUntil, generateVehicleDocs, type VehicleDocRow } from "../_helpers";
import {
...
export function VehicleDocumentsTab({ vehicle }: { vehicle: Vehicle }) {
  const docs = useMemo(
    () => generateVehicleDocs(vehicle.id, vehicle.licensePlate),
  ...
  const columns: Column<VehicleDocRow>[] = [
    {
      key: "type",
      header: "Document",
      sortable: true,
    ...
    },
    {
      ...
      render: (r) => {
        ...
        const weight = r.status === "Expired" ? 0 : r.status === "Expiring Soon" ? 1 : 2;
        return (
    ...
    },
    {
      ...
      render: (r) => {
        ...
        return <StatusBadge variant={variant} pulse={pulse}>{r.status}</StatusBadge>;
      },
    },
  ];
        ...
        {expiredCount > 0 && (
          <div className="mb-3 flex items-start gap-2 rounded-[5px] border border-foreground/30 bg-background p-3">
            ...
            <div className="text-[12px] text-foreground">
              ...
              <span className="text-muted-foreground">Vehicle may be detained at RTO check-posts until renewed.</span>
            </div>
          </div>
        )}
        <DataTable
          ...
          emptyDescription="Upload the RC, insurance, fitness, permit, and PUC to begin."
        />
      </SectionCard>
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-[440px] rounded-[6px]">
        <DialogHeader>
          <DialogTitle className="text-[16px] font-medium">Upload Vehicle Document</DialogTitle>
          <DialogDescription className="text-[12px] text-muted-foreground">
            Choose the document type and attach a scan.
          </DialogDescription>
        </DialogHeader>
        <div className="flex flex-col gap-3 py-2">
          <div className="flex flex-col gap-1.5">
            ...
            <Select value={docType} onValueChange={setDocType}>
              ...
              <SelectContent>
                {[
                  "RC Book", "Insurance Certificate", "Fitness Certificate", "National Permit",
                  "State Permit", "Pollution Certificate (PUC)", "Tax Receipt",
                ].map((t) => (
