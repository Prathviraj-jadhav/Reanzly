// Seeds real Approval requests - the tenant-facing Approvals module
// previously ran entirely on a client-only APPROVAL_REQUESTS mock array
// (approval-detail.tsx searched that same static array independently of
// the list's live state, so a decision made from the detail view never
// showed up back in the list - the same detached-detail-view bug already
// fixed in Field Service/Helpdesk/Knowledge). Migrates the original
// hand-authored 18-record set (captured here before its export was removed
// from the module's own _helpers.tsx) into real db.approvalRequest rows.
//
// Idempotent: skips if this company already has ApprovalRequest rows.
// Run with: bun run src/scripts/seed-approvals.ts
...
interface ApproverStep {
  ...
  role: string;
  order: number;
  state: "Pending" | "Approved" | "Rejected" | "Delegated" | "Skipped";
  decisionAt?: string;
  comment?: string;
  delegatedTo?: string;
}
...
const APPROVAL_REQUESTS: SeedApprovalRequest[] = [
  {
    requestId: "APR-1001",
    type: "Expense",
...
async function main() {
  console.log("[seed-approvals] starting...");

  const existing = await db.approvalRequest.count({ where: { companyId: COMPANY_ID } });
  if (existing > 0) {
    console.log(`[seed-approvals] already seeded (${existing} requests found) - skipping.`);
    return;
  }
  ...
  for (const a of APPROVAL_REQUESTS) {
    await db.approvalRequest.create({
      data: {
        ...
        relatedRef: a.relatedRef ?? null,
        payloadJson: JSON.stringify(a.payload),
        approversJson: JSON.stringify(a.approvers),
  ...
  console.log(`[seed-approvals] seeded ${created} real approval requests.`);
}

main()
  .catch((e) => {
    console.error("[seed-approvals] failed:", e);
    process.exitCode = 1;
  })
