...
export async function GET() {
  ...
  const data = await db.warehousePodReceive.findMany({
...
export async function POST(req: NextRequest) {
  ...
  try {
    const created = await db.warehousePodReceive.create({
      data: warehouseCreateMappers.podReceive(body, sessionUser.companyId),
    });
    return NextResponse.json({ receive: created }, { status: 201 });
  } catch (e: any) {
