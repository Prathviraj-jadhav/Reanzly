...
export async function GET() {
  ...
  const data = await db.warehouseOutbound.findMany({
...
export async function POST(req: NextRequest) {
  ...
  try {
    const created = await db.warehouseOutbound.create({
      data: warehouseCreateMappers.outbound(body, sessionUser.companyId),
    });
