...
| 11 | **Entire CRM cluster** | No role's `permissions[]` lists the literal `"crm"` string except owner's wildcard, and the sidebar's client-side `canAccess()` has no parent-expansion (unlike the server-side check). Result: Customers, Vendors, Helpdesk, Marketing, and Surveys — all tabs inside the CRM cluster page — are **unreachable for every non-owner role**, including `broker`, who is explicitly granted `"customers"`/`"vendors"` permissions that go nowhere without also holding `"crm"`. Same bug class as Automation (#1), affecting five modules at once. | 4 |
