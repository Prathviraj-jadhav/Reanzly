...
| 2 | ~~**Invoice**~~ | **Fixed 2026-08-12.** "Cancel Invoice" (list row action and detail-view quick action) now calls the real `onUpdateStatus`/`onUpdate` handlers (existing `PATCH /api/invoices/[id]`) before showing the toast, instead of only showing the toast. Verified live: cancelled an invoice as `finance-manager`, confirmed `status: "Cancelled"` persisted via a fresh `GET /api/invoices` fetch (not just optimistic UI), then restored the seed invoice's original status. | 1 |
...
**Helpdesk — now fully real, fixed 2026-08-12.** Given a real `HelpdeskTicket` Prisma model + API (`GET/POST /api/helpdesk`, `GET/PATCH /api/helpdesk/[id]`) — a new model deliberately kept separate from `SupportTicket` (that one is the customer-facing Vendor Portal's ticket model: required `customerId` FK, a different status vocabulary, its own ticket-numbering sequence, and no SLA/team-routing/internal-note fields Helpdesk needs). `ticket-detail.tsx` now fetches its ticket by id directly instead of re-deriving from the static `HELPDESK_TICKETS` array, fixing the "not found" bug; status changes and replies/internal notes now persist via `PATCH` (previously local-only `useState`) with a real activity-log entry stamped with the signed-in user's name. The list's "Mark resolved" row action now calls the same real update instead of only showing a toast. Genuinely un-designed actions (Reassign/Escalate dialogs, Edit, Log call) were left as toast-only — no UI exists yet to pick a value for them, same restraint applied to Field Service/Knowledge's leftover secondary actions. Seeded via `seed-helpdesk.ts` (reuses the original 25-ticket generator, kept in `_helpers.tsx` as seed source). Verified live as `broker` (who already holds `crm` from the earlier CRM cluster fix): created a new ticket, opened it (no "not found"), changed its status, confirmed via a fresh `GET` that both persisted with the real actor name, then removed the test ticket.
