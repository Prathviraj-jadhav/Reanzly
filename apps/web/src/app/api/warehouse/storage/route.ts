...
export async function GET() {
  ...
  const data = await db.warehouseStorageLocation.findMany({
...
export async function POST(req: NextRequest) {
  ...
  try {
    const created = await db.warehouseStorageLocation.create({
      data: warehouseCreateMappers.storage(body, sessionUser.companyId),
    });
